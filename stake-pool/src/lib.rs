...
use near_sdk::{env, ext_contract, near_bindgen, AccountId, Balance, EpochHeight, Promise, PromiseResult, PublicKey, Gas};
...
impl Default for Account {
    fn default() -> Self {
        Self { unstaked: 0, stake_shares: 0, unstaked_available_epoch_height: 0, stake_principal: 0, }
    }
}
...
impl RewardFeeFraction {
    pub fn assert_valid(&self) {
        ...
        assert!(self.numerator <= self.denominator, "The reward fee must be less or equal to 1");
    }
...
mod tests {
    ...
    fn zero_fee() -> RewardFeeFraction {
        RewardFeeFraction { numerator: 0, denominator: 1 }
    }
    ...
    fn test_deposit_withdraw() {
        ...
        emulator.update_context(bob(), 0);
        assert_eq!(emulator.contract.get_account_unstaked_balance(bob()).0, deposit_amount);
        emulator.contract.withdraw(deposit_amount.into());
        assert_eq!(emulator.contract.get_account_unstaked_balance(bob()).0, 0u128);
    }
